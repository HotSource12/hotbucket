...
public class Clear_Activity extends ImageSelectHelperActivity {
	...
	private int mMonth;
	private int mDay;
	static final int DATE_DIALOG_ID = 0; // 날짜를 넣어줍쇼
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		selectphoto = (ImageView) findViewById(R.id.ivImageSelected); // 사진이미지
		selectphoto.setOnClickListener(new OnClickListener() { // 사진이미지 누르면
																// 갤러리로연결
					public void onClick(View v) {
						// TODO Auto-generated method stub
						startSelectImage();
						setImageSizeBoundary(200);// 사진싸이즈
						setCropOption(100,100);// 크롭하자
					}
				});

		photouri = Uri.fromFile(getSelectedImageFile()); // 사진파일을 uri로 바꿔서변수저장
		...
		okBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				Toast.makeText(getApplicationContext(), "완료", 1000).show();
		...
		resetBtn.setOnClickListener(new OnClickListener() { // 완료창에서 취소 누를 경우
			public void onClick(View v) {
				Toast.makeText(getApplicationContext(), "취소", 1000).show();
				onBackPressed();
			}
		});
